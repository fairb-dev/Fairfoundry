...
enum ContractStatus {
  ...
  PRODUCTION     // Live submissions and review windows
...
model Submission {
  ...
  verifiedAt    DateTime?
  reviewDeadline DateTime? // submittedAt + 48 hours
  status        String    // VERIFYING, VERIFIED, UNDER_REVIEW, FINALIZED
  contract      Contract  @relation(fields: [contractId], references: [id])
  results       UnitResult[]
...
model FinalizationRecord {
  id            String     @id @default(cuid())
  submissionId  String     @unique
  contractId    String
  finalizedAt   DateTime?
  status        String     // PENDING, FINALIZED, UNDER_REVIEW
  submission    Submission @relation(fields: [submissionId], references: [id])
  contract      Contract   @relation(fields: [contractId], references: [id])
}
```
...
/studio/[contractId]/production → Live submissions and review windows
...
PRODUCTION
  ...
  ↓ 48-hour customer review window
  ↓ Finalization or review protocol
  ↓ Quality credentials issued on validated completion
```
