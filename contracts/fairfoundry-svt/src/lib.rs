...
//! ## Design
//!
//! - **Mint authority**: Only the Fairfoundry control contract can mint.
...
pub enum SvtErr {
    /// Caller is not the authorized minter (control contract).
    NotAuthorized = 1,
...
/// Development stage mirroring the control contract's ServiceStage.
...
#[contracttype]
pub struct SvtState {
    /// The control contract authorized to mint SVTs.
    pub minter: Address,
...
#[contractimpl]
impl FairfoundrySvt {
    /// Initializes the SVT contract with the authorized minter (control contract).
    ///
    /// # Arguments
    /// * `admin` - Admin address (must authorize)
    /// * `minter` - The Fairfoundry control contract address authorized to mint
    ...
    /// Mints a new SVT. Only the authorized minter (control contract) may call this.
    ///
    /// # Arguments
    /// * `svt_id` - Unique SVT ID (assigned by control contract)
    ...
    pub fn mint(
        ...
        // Only the control contract can mint
